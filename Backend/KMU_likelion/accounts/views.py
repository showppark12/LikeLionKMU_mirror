...
class UserViewSet(viewsets.ModelViewSet):
    ...
    action_serializer_classes = {
        "activity": serializers.UserActivitySerializer}
    ...
    def get_submit_status(self, request, *args, **kwargs):
        ...
        get_session = Session.objects.get(
            id=get_assignment.lecture.id)  # 해당 lecture 객체 가져오기
        #    if get_user.start_number == get_session.start_number: # 해당 기수 멤버의 세션인지 확인 작업
        try:
             user_submit = Submission.objects.get(
                 user_id=get_user, lecture=assignment_id)
             ...
             if int(user_split[0]) > int(session_split[0]):  # 년도 비교
                    return Response({'status': 'LATE'})
                       ...
                       if int(user_split[1]) > int(session_split[1]):  # 달 비교
                           ...
                           # 일 비교
                           if int(user_split[1]) == int(session_split[1]) and int(user_split[2]) > int(session_split[2]):
                               return Response({'status': 'LATE'})
